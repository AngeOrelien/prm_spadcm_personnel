...
/// (envoi + réception) — remplace `MessagerieStubPage` pour le
/// coordonnateur. [conversationId] est un vrai id de conversation, obtenu
/// via `CoordonnateurActions.ouvrirConversationAvec(...)` avant la
/// navigation (voir `coordonnateur_messagerie_page.dart`,
/// `coordonnateur_avs_detail_page.dart`, `coordonnateur_patient_detail_page.dart`).
class CoordonnateurConversationPage extends ConsumerStatefulWidget {
  final String conversationId;
  final String interlocuteurNom;
  final String? interlocuteurSousTitre;

  const CoordonnateurConversationPage({
    super.key,
  ...
  @override
  ConsumerState<CoordonnateurConversationPage> createState() => _CoordonnateurConversationPageState();
}

class _CoordonnateurConversationPageState extends ConsumerState<CoordonnateurConversationPage> {
  ...
  void initState() {
    super.initState();
    // Marque le fil comme lu dès l'ouverture.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(coordonnateurActionsProvider).marquerConversationLue(widget.conversationId);
    });
    // Rafraîchissement périodique pour simuler la réception de nouveaux
    // messages (pas de websocket côté backend pour l'instant) — voir
    // BACKEND_TODO.md pour le passage à du temps réel.
  ...
  Future<void> _envoyer() async {
    ...
    _saisieCtrl.clear();
    try {
      await ref.read(coordonnateurActionsProvider).envoyerMessage(widget.conversationId, texte);
      if (!mounted) return;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        children: [
          Expanded(
            child: messagesAsync.when(
              ...
              error: (err, st) => _ErreurMessages(onReessayer: () => ref.invalidate(messagesProvider(widget.conversationId))),
              data: (messages) {
                if (messages.isEmpty) {
                  return Center(
